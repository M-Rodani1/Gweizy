...
const TransactionPilot: React.FC<TransactionPilotProps> = ({ ethPrice = 3000 }) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.recommendation.action === 'WAIT') {
          // Estimate wait time based on predictions
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-2xl border border-gray-700/50 overflow-hidden">
            ...
            <button
              ...
              className={`
                ...
                ${selectedTxType === type
                  ...
                  : 'bg-gray-800 border-2 border-transparent text-gray-300 hover:border-gray-600'
                }
              `}
            >
              <span>{icon}</span>
      ...
      <div className="p-6">
        {loading && !recommendation ? (
          ...
          <div className={`relative rounded-xl p-6 bg-gradient-to-r ${actionConfig.gradient} bg-opacity-10 border border-white/10 shadow-lg ${actionConfig.bgGlow}`}>
            ...
            {/* Main action */}
            <div className="mb-4">
              <div className="text-3xl font-bold text-white mb-1">{actionConfig.text}</div>
              ...
              <div className="bg-black/20 rounded-lg p-3">
                ...
                <div className="text-lg font-mono font-bold text-green-400">
                  {recommendation?.expected_savings
                    ? `$${(recommendation.expected_savings * estimatedCostUsd).toFixed(4)}`
                    : '-'
                  }
                </div>
              </div>
      ...
      {bestChainForTx && bestChainForTx.chainId !== selectedChain.id && bestChainForTx.savings > 10 && (
        <div className="mx-6 mb-4 p-3 bg-green-500/10 border border-green-500/30 rounded-xl">
          <div className="flex items-center justify-between">
            <div className="text-sm text-green-400">
              Save {bestChainForTx.savings.toFixed(0)}% by using {bestChainForTx.reason.split(' ')[0]}
            </div>
            <button className="text-xs px-3 py-1 bg-green-500/20 text-green-400 rounded-lg hover:bg-green-500/30 transition-colors">
              Switch Chain
            </button>
          </div>
        </div>
