...
class GasOptimizationEnv:
    ...
    def reset(self, urgency: Optional[float] = None) -> np.ndarray:
        ...
        self._episode_data = episodes[0] if episodes else self.data_loader._generate_synthetic_data(24)[:self.episode_length]
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        if action == 1:  # Executed
            ...
            if self._current_step >= len(self._episode_data) - 1 or self._time_waiting >= self.max_wait_steps:
                self._done = True
                # Force execution penalty
                reward += self.reward_calculator.calculate_reward(1, current_price, self._urgency, self._time_waiting, done=True)
